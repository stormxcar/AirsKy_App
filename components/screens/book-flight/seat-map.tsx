...
type SeatMapProps = {
  ...
  className?: string; // Thêm className để xác định hạng vé
};
...
const SeatItem = React.memo(
  ({
    ...
    const handleLongPress = () => {
      ...
      const priceText =
        item.price > 0 ? `Phụ phí: +${priceString} ₫` : "Miễn phí";
      Alert.alert(
    ...
    return (
      <Pressable onLongPress={handleLongPress}>
        <TouchableOpacity
          ...
          style={[styles.seatBase, seatStyle] as any}
          activeOpacity={0.7}
        >
          <Text
            style={[
              styles.seatText,
              item.status === SeatStatus.OCCUPIED && styles.seatTextOccupied,
            ]}
          >
            {item.seatNumber}
          </Text>
...
const SeatMap = ({
  ...
  const renderRow = ({
    ...
    item: { rowNumber: string; seats: Seat[] };
  }) => (
        ...
        ListFooterComponent={
          <>
            ...
            <View className="mt-6 space-y-2">
              <Text className="text-base font-bold text-center text-blue-900 mb-2">
                Chú thích
              </Text>
              ...
              <View className="flex justify-start gap-x-4 gap-y-2 p-5">
                ...
                <View className="flex-row items-center">
                  <View
                    style={[styles.legendBox, { backgroundColor: "#dc2626" }]}
                  />
                  <Text className="text-xs text-gray-600">Đã đặt</Text>
                </View>
                <View className="flex-row items-center">
                  <View
                    style={[styles.legendBox, { backgroundColor: "#e5e7eb" }]}
                  />
                ...
                <View className="flex-row items-center">
                  <View
                    style={[
                      styles.legendBox,
                      { backgroundColor: "white", borderColor: "#1e40af" },
                    ]}
                  />
                  <Text className="text-xs text-gray-600">
                    Tiêu chuẩn (+
                    {SEAT_TYPE_PRICES.STANDARD.toLocaleString("vi-VN")}đ)
                  </Text>
                </View>
                <View className="flex-row items-center">
                  <View
                    style={[
                      styles.legendBox,
                      { backgroundColor: "#fef08a", borderColor: "#facc15" },
                    ]}
                  />
                  <Text className="text-xs text-gray-600">
                    Hàng đầu{" "}
                    {className === "Business" || className === "First"
                      ? "(Miễn phí)"
                      : `(+${SEAT_TYPE_PRICES.FRONT_ROW.toLocaleString("vi-VN")}đ)`}
                  </Text>
                </View>
                <View className="flex-row items-center">
                  <View
                    style={[
                      styles.legendBox,
                      { backgroundColor: "#fecaca", borderColor: "#f87171" },
                    ]}
                  />
                  <Text className="text-xs text-gray-600">
                    Lối thoát hiểm{" "}
                    {className === "Business" || className === "First"
                      ? "(Miễn phí)"
                      : `(+${SEAT_TYPE_PRICES.EXIT_ROW.toLocaleString("vi-VN")}đ)`}
                  </Text>
                </View>
                <View className="flex-row items-center">
                  <View
                  ...
                  />
                  <Text className="text-xs text-gray-600">
                    Thêm không gian chân{" "}
                    {className === "Business" || className === "First"
                      ? "(Miễn phí)"
                      : `(+${SEAT_TYPE_PRICES.EXTRA_LEGROOM.toLocaleString("vi-VN")}đ)`}
                  </Text>
                </View>
                <View className="flex-row items-center">
                  <View
                    style={[
                      styles.legendBox,
                      { backgroundColor: "#cffafe", borderColor: "#22d3ee" },
                    ]}
                  />
                  <Text className="text-xs text-gray-600">
                    Ghế ưu tiên{" "}
                    {className === "Business" || className === "First"
                      ? "(Miễn phí)"
                      : `(+${SEAT_TYPE_PRICES.ACCESSIBLE.toLocaleString("vi-VN")}đ)`}
                  </Text>
                </View>
              </View>
            </View>
