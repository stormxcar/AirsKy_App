        ...
        <View className="bg-white rounded-xl p-4 mb-4 border border-gray-200 shadow-sm">
            ...
            <View className=" py-3 border-b border-gray-100 gap-2">
                ...
                {BAGGAGE_PACKAGES.map((pkg) => {
                    ...
                    return (
                        <TouchableOpacity key={pkg.key} onPress={() => onBaggageChange(isSelected ? null : pkg)} className={`p-4 border-2 rounded-xl flex-row justify-between items-center ${isSelected ? 'border-blue-950 bg-blue-50' : 'border-gray-300'}`}>
                            <Text className={`font-bold ${isSelected ? 'text-blue-950' : 'text-gray-700'}`}>{pkg.label}</Text>
